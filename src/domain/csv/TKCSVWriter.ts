...
function computeExportFilename(dataset: TKDatasetFilterer, submissionIdRaw: string): string {
    if (dataset) {
...
function computeCSVContent(submission: TKSubmission, locale: string): string {
        if (submission) {
            ...
            for (const submissionItem in submission.thematics[thematic]
                .data) {
                const item = submission.thematics[thematic].data[
                submissionItem
                ];

                const itemName = TKGetLocalValue(
                item.fieldLabel,
                locale
                );

                if (item instanceof TKSubmissionEntryText) {
                const answer = TKGetLocalValue(
                    item.answerLabel,
                    locale
                );
                const trafficlight = item.trafficLight
                    ? item.trafficLightColor
                    : "";
...
export function TKCSVWrite(dataset: TKDatasetFilterer, submission: TKSubmission, submissionIdRaw: string, locale: string){
    const csvContent = computeCSVContent(submission, locale);
    console.log(csvContent);
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", computeExportFilename(dataset,submissionIdRaw)); // filename
